...
- **G-8. CLOSED 2026-04-19.** Framework claims reflexivity (§7, §49). Fixed by writing `docs/TELOS_ON_TELOS.md` — a live declaration of the Telos project in Framework v0.2.1 terms (Vision → Mission → Strategic → Tactical → 4 Operational leaves with full Contracts including `causal_mechanism`, Requirements, Invariants, Scope, Reproducibility). Dog-food observations in §10 of that file surfaced G-5 (parent-Contract aggregation) in practice and confirmed that `causal_mechanism`, `Metric.Balancing`, failure policies, and Dependencies hold up under real-world use. Original task still in §3.2 #3.
